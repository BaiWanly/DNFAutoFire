...
;@Ahk2Exe-Let AppVersion = 0.21
;@Ahk2Exe-SetMainIcon gui\icon_main.ico
;@Ahk2Exe-AddResource gui\icon_alert.ico, 160
;@Ahk2Exe-AddResource gui\icon_green.ico, 206
;@Ahk2Exe-AddResource gui\icon_red.ico, 207
...
global __Version := GetAppVersion()
...
GetAppVersion() {
    if (A_IsCompiled) {
        try {
            ...
            if (version != "") {
                return RegExReplace(version, "^(\d+\.\d+)(?:\.0)+$", "$1")
            }
        }
    }
    try source := FileRead(A_ScriptFullPath, "UTF-8")
    catch {
        source := ""
    }
    if RegExMatch(source, "m)^;@Ahk2Exe-Let\s+AppVersion\s*=\s*([^\r\n]+)", &match) {
        return Trim(match[1])
    }
    return "0.0"
}
...
A_IconTip := "DAF连发工具"
try TraySetIcon(A_IsCompiled ? A_ScriptFullPath : A_ScriptDir "\gui\icon_main.ico", 1)
