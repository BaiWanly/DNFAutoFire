...
global exText := Map(
    ...
    "ComboHelp", "在 DNF 窗口内按住「触发按钮」后，按顺序发送技能键。`n可配置多套方案；未设置触发键、触发键无效或没有有效技能的方案会在运行时跳过。`n勾选「循环」时，整套打完后若仍按住触发按钮会重新开始。`n勾选「屏蔽原键」后，触发键自身不会透传给游戏。`n技能键留空（按 ESC）会作为「空技能」保留，仅按其延迟占位，不发送按键。",
