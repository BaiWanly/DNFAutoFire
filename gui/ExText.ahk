...
global exText := Map(
    ...
    "AutoRunHelp", "设置自动奔跑要监听的左右键，以及按住方向键后补发双击的延迟（毫秒）。`n默认 30ms；范围 1～400ms。`n如果游戏里方向键不是 Left/Right，请改成你的实际按键后保存。`n自动奔跑受软件设置中的全局暂停快捷键控制。",
    ...
    "ComboHelp", "在 DNF 窗口内按住「触发按钮」后，按顺序发送技能键。`n可配置多套方案；未设置触发键、触发键无效或没有有效技能的方案会在运行时跳过。`n勾选「循环」时，整套打完后若仍按住触发按钮会重新开始。`n勾选「屏蔽原键」后，触发键自身不会透传给游戏。`n双击技能进入编辑对话框后清空按键，会作为「空技能」保留，仅按其延迟占位，不发送按键。",
