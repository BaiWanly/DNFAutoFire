...
; 使用 WS_EX_LAYERED + 整窗 alpha：半透明白色叠层，便于透过框对准游戏
...
PresetRegionPickApplyDwmStyle(hwnd) {
    ...
    try {
        NumPut("uint", 0x00FFFFFF, val, 0) ; FFFFFF，与 BackColor 一致
...
PresetRegionPickOpen(kind := "skill") {
    ...
    ; 半透明白色叠层；灰边靠 WM_NCCALCSIZE 吃掉 ThickFrame 非客户区
    ...
    PresetRegionPickApplyDwmStyle(hwnd)
    ; 255=不透明；数值越高白色越实，越低越透（相对原 125 降低透明度=更不透明）
