...
global AutoPresetsText := Map(
    ...
    "Save", "保存",
    "HelpTitle", "自动识别说明",
    "Help", "按 Esc 或附加键会触发搜图。启动连发或按下快捷键后开始识别，并自动切换角色，只要不是纯鼠标切换角色都能自动识别。`n`n流程：框选地下城识别区域并截取地下城参考图；选择分辨率后，为各配置绑定当前分辨率下的技能图（双击可改名）。点击截取会自动为当前配置和分辨率新增一张技能图。匹配到对应配置才切换，技能识别失败不切换。",
    "SkillRegion", "技能识别区域",
